...
def login():
    ...
    try:
        ...
        user_dict = user if isinstance(user, dict) else {
            ...
            'verified': user[5] if len(user) > 5 else 0
        }
        ...
        is_verified = str(user_dict.get('verified', 0)) == '1' or user_dict.get('verified', 0) == 1
        if not is_verified:
            cursor.close()
            conn.close()
            return jsonify({
...
def get_projects():
    ...
    try:
        ...
        if rows and isinstance(rows[0], dict):
            ...
            projects = [dict(row) for row in rows] if rows else []
...
def create_project():
    ...
    try:
        ...
        attendance_mode = data.get('attendance_mode', 'daily').lower()
...
def verify_otp_endpoint():
    ...
    try:
        ...
        if existing_user:
            ...
            is_verified = existing_user['verified'] if isinstance(existing_user, dict) else existing_user[1]
...
def resend_otp_endpoint():
    ...
    try:
        ...
        if pending and pending['email'] == email:
            ...
            if user:
                is_verified = user['verified'] if isinstance(user, dict) else user[0]
                if not is_verified:
                    otp = create_otp(email)
